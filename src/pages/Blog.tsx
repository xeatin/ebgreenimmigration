...
import { useTranslatedSummaries } from "@/hooks/useTranslatedPost";

const Blog = () => {
  const { lang } = useLanguage();
  const reversed = useMemo(() => [...blogPosts].reverse(), []);
  const { posts, loading } = useTranslatedSummaries(reversed, lang);
    ...
    <div className="min-h-screen bg-green-deep">
      ...
      <section className="pt-8 pb-20 bg-green-deep">
        <div className="container mx-auto px-6">
          {loading && lang !== "pt" && (
            <div className="flex items-center justify-center gap-2 text-cream/60 font-body text-sm mb-6">
                ...
                >
                  ...
                  <div className="p-6 flex flex-col flex-1">
                    ...
                    <span className="btn-highlight inline-flex items-center justify-center gap-2 bg-gradient-gold text-green-deep px-6 py-3 rounded-md font-bold text-sm font-body group-hover:opacity-90 transition-opacity shadow-card group-hover:shadow-card-hover w-full">
                      {t(translations.blog.readArticle, lang)}
          ...
          {blogPosts.length === 0 && (
            <div className="text-center py-20">
              <p className="font-body text-cream/50 text-lg">
                {t(translations.blog.empty, lang)}
              </p>
            </div>
          )}
        </div>
        ...
        <div className="container mx-auto px-6 text-center">
          ...
          >
            <h2 className="font-display text-3xl md:text-4xl font-bold text-cream mb-4">
              {t(translations.blog.ctaTitle1, lang)}{" "}
              <span className="text-gradient-gold">{t(translations.blog.titleHighlight, lang)}</span>{" "}
              {t(translations.blog.ctaTitle2, lang)}
            </h2>
            <p className="text-cream/70 font-body text-lg mb-8">
              {t(translations.blog.ctaText, lang)}
            </p>
            <button
              type="button"
